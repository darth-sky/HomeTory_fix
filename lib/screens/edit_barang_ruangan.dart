...
class EditBarangRuangan extends StatefulWidget {
  final BarangDlmRuangan barangDlmRuangan;
  ...
  @override
  State<EditBarangRuangan> createState() => _EditBarangRuanganState();
}
...
class _EditBarangRuanganState extends State<EditBarangRuangan> {
  ...
  void initState() {
    ...
    _barangController = TextEditingController(
        text: widget.barangDlmRuangan.namaBarangDlmRuangan);
    _descItemController = TextEditingController(
        text: widget.barangDlmRuangan.descBarangDlmRuangan);
    _itemQtyController = TextEditingController(
        text: widget.barangDlmRuangan.qntyBarangDlmRuangan.toString());
    _selectedCategory = widget.barangDlmRuangan.categoryBarangDlmRuangan;
  }
    ...
    Navigator.pushReplacement(
      ...
      MaterialPageRoute(
        builder: (context) => InsideBarangDlmRuangan(
          idRuangan: widget.barangDlmRuangan.idRuangan,
          idInsideBarangDlmRuangan: widget.barangDlmRuangan.idBarangDlmRuangan,
          currentPages: 1,
        ),
      ),
    );
  ...
  Future<void> _updateDataWithImage(
    ...
    try {
      var request = http.MultipartRequest(
          'PUT',
          Uri.parse(
              '${Endpoints.barangDlmRuanganUpdate}/${widget.barangDlmRuangan.idBarangDlmRuangan}'));
                  ...
                  children: [
                            ...
                            child: Container(
                              ...
                              child: galleryFile == null
                                  ? (widget.barangDlmRuangan
                                          .gambarBarangDlmRuangan.isNotEmpty
                                      ? Image.network(
                                          "${Endpoints.baseUAS}/static/img/${widget.barangDlmRuangan.gambarBarangDlmRuangan}")
                                      : Center(
                                          child: Row(
                                            mainAxisAlignment:
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        String qntyBarang = _itemQtyController.text;
                        String descBarang = _descItemController.text;
                        if (barangRuanganName.isNotEmpty &&
                            // galleryFile != null &&
                            qntyBarang.isNotEmpty &&
                            descBarang.isNotEmpty) {
                          final accessToken =
                              context.read<AuthCubit>().state.accessToken;
                          _updateDataWithImage(
                              context,
                              widget.barangDlmRuangan.idBarangDlmRuangan,
                              accessToken!);
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
