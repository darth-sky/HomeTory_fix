...
class EditRuangan extends StatefulWidget {
  ...
  const EditRuangan({required this.ruangan, super.key});
...
class _EditRuanganState extends State<EditRuangan> {
  ...
  void initState() {
    super.initState();
    _titleController = TextEditingController(text: widget.ruangan.namaRuangan);
  }
  ...
  Future<void> _updateDataWithImage(BuildContext context, String accessToken) async {
    try {
      var request = http.MultipartRequest('PUT',
          Uri.parse('${Endpoints.ruanganUpdate}/${widget.ruangan.idRuangan}'));
          request.headers['Authorization'] = 'Bearer $accessToken';
      request.fields['id_ruangan'] = widget.ruangan.idRuangan.toString();
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: galleryFile == null
                                  ? (widget.ruangan.gambarRuangan.isNotEmpty
                                      ? Image.network(
                                          "${Endpoints.baseUAS}/static/img/${widget.ruangan.gambarRuangan}")
                                      : Center(
                                          child: Row(
                                            mainAxisAlignment:
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        if (ruanganName.isNotEmpty) {
                          ...
                          _updateDataWithImage(context, accessToken!);
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
