...
class EditContainer extends StatefulWidget {
  ...
  const EditContainer({required this.containers, Key? key}) : super(key: key);
...
class _EditContainerState extends State<EditContainer> {
  ...
  void initState() {
    super.initState();
    _titleController = TextEditingController(text: widget.containers.nama_containers);
  }
  ...
  Future<void> _updateDataWithImage(BuildContext context, int idUser) async {
    try {
      var request = http.MultipartRequest('POST',
          Uri.parse('${Endpoints.containerUpdate}/${widget.containers.id_containers}'));
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => InsideContainer(idInsideContianer: widget.containers.id_containers, idRuangan: widget.containers.id_ruangan,)
          ),
        );
      } else {
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: galleryFile == null
                                  ? (widget.containers.gambar_containers.isNotEmpty
                                      ? Image.network(
                                          "${Endpoints.baseUAS}/static/img/${widget.containers.gambar_containers}")
                                      : Center(
                                          child: Row(
                                            mainAxisAlignment:
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        if (ruanganName.isNotEmpty) {
                          _updateDataWithImage(context, widget.containers.id_containers);
                        }
                      },
